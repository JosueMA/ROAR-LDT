...
# real-pseudo
lme1 <- lmer(rt.log ~ realpseudo + (1 | subj) + (1 | word), filter(rt.data,acc==1))
summary(lme1)
# Lexical frequency real
lme2 <- lmer(rt.log ~ log(FREQ) + (1 | subj) + (1 | word), filter(rt.data,acc==1 & realpseudo=='real'))
summary(lme2)
# Bigram frequency pseudo
lme3 <- lmer(rt.log ~ log(UN2_F) + (1 | subj) + (1 | word), filter(rt.data,acc==1 & realpseudo=='pseudo'))
summary(lme3)
# Word length effect
lme4 <- lmer(rt.log ~ scale(wordLength) * + (1 | subj) + (1 | word), filter(rt.data,acc==1))
summary(lme4)
# Word length effect interaction
lme5 <- lmer(rt.log ~ scale(wordLength) * scale(wj_lwid_raw) + (1 | subj) + (1 | word), filter(rt.data,acc==1))
summary(lme5)

# Bigram frequency real
lme6 <- lmer(rt.log ~ log(UN2_F) + (1 | subj) + (1 | word), filter(rt.data,acc==1 & realpseudo=='real'))
summary(lme6)
